...
public class ArticleQueryActivity extends Activity {
	// 声明查询按钮
	...
	/*查询过滤条件保存到这个对象中*/
	private Article queryConditionArticle = new Article();
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.article_query);
		ImageView search = (ImageView) this.findViewById(R.id.search);
		search.setVisibility(View.GONE);
		TextView title = (TextView) this.findViewById(R.id.title);
		title.setText("设置文章查询条件");
		...
		articleClassObj_ShowText = new String[articleClassCount+1];
		articleClassObj_ShowText[0] = "不限制";
		for(int i=1;i<=articleClassCount;i++) { 
			articleClassObj_ShowText[i] = articleClassList.get(i-1).getArticleClassName();
		} 
		...
		spinner_articleClassObj.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1,int arg2, long arg3) {
				if(arg2 != 0)
					queryConditionArticle.setArticleClassObj(articleClassList.get(arg2-1).getArticleClassId()); 
				else
		...
		spinner_userObj.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1,int arg2, long arg3) {
				if(arg2 != 0)
					queryConditionArticle.setUserObj(userInfoList.get(arg2-1).getUser_name()); 
				else
					queryConditionArticle.setUserObj("");
			}
			@Override
			public void onNothingSelected(AdapterView<?> arg0) {}
		});
		...
		btnQuery.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				try {
					/*获取查询参数*/
					queryConditionArticle.setTitle(ET_title.getText().toString());
					queryConditionArticle.setAddTime(ET_addTime.getText().toString());
					Intent intent = getIntent();
					//这里使用bundle绷带来传输数据
					Bundle bundle =new Bundle();
					//传输的内容仍然是键值对的形式
					bundle.putSerializable("queryConditionArticle", queryConditionArticle);
