...
/*需要绑定的数据*/
  ...
  public View getView(int position, View convertView, ViewGroup parent) { 
	  ...
	  /*设置各个控件的展示内容*/
	  holder.tv_title.setText("帖子标题：" + mData.get(position).get("title").toString());
	  holder.tv_hitNum.setText("浏览量：" + mData.get(position).get("hitNum").toString());
	  holder.tv_userObj.setText("发帖人：" + (new UserInfoService()).GetUserInfo(mData.get(position).get("userObj").toString()).getName());
	  holder.tv_addTime.setText("发帖时间：" + mData.get(position).get("addTime").toString());
	  /*返回修改好的view*/
	  return convertView; 
    } 
