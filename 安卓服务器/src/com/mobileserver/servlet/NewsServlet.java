	...
	protected void doPost(HttpServletRequest request,
		...
		if (action.equals("query")) {
			...
			for (int i = 0; i < newsList.size(); i++) {
				...
				.append("		<viewNum>")
				.append(newsList.get(i).getViewNum())
				.append("</viewNum>").append("\r\n")
				.append("		<publishDate>")
			...
			try {
			  ...
			  stringer.key("viewNum").value(news.getViewNum());
			...
			int viewNum = Integer.parseInt(request.getParameter("viewNum"));
			...
			/*更新新闻之前先根据newsId查询某个新闻*/
			int newsId = Integer.parseInt(request.getParameter("newsId"));
			News news = newsDAO.GetNews(newsId);

			// 客户端查询的新闻对象，返回json数据格式, 将List<Book>组织成JSON字符串
			JSONStringer stringer = new JSONStringer(); 
			try{
			  stringer.array();
			  stringer.object();
